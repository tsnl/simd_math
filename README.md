...
Matrices are constructed in column-major order, i.e. each argument to `SimdAffine3::new` is a column. This is consistent with OpenGL and GLM, but not with most textbook math. `SimdAffine3` is a 3×4 affine transform: three transformed basis-vector columns plus a translation column, with an implicit `[0, 0, 0, 1]` bottom row.
